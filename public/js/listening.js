...
async function startListeningPractice() {
  if (isListeningPracticeActive) {
    addMessage(
      "⚠️ Listening practice already in progress. Complete it first.",
      "system-error",
    );
    return;
  }
  ...
  addMessage(
    "I'll speak a sentence. Listen carefully, then repeat it back.",
    "system-success",
  );
  ...
  try {
    ...
    setTimeout(() => {
      ...
      addMessage(
        "🎤 **Now it's your turn!** Speak the sentence you heard. Click the microphone button when ready.",
        "system-success",
      );
      ...
      addMessage(
        "⬅️ Click the 🎤 microphone button and repeat the sentence.",
        "system-success",
      );
    ...
    addMessage(
      "⚠️ Failed to generate listening practice sentence. Please try again.",
      "system-error",
    );
  }
}
...
export async function evaluateListeningAttempt(
  attempt,
  targetSentence,
  targetLanguage,
) {
  ...
  try {
    ...
    addMessage(
      "🔄 Ready for another listening practice? Click the 🎧 button again!",
      "system-success",
    );
  } catch (error) {
    const typingElem = document.getElementById("typing-indicator");
    ...
    addMessage(
      "⚠️ Error evaluating your attempt. The correct sentence was: " +
        targetSentence,
      "system-error",
    );
  } finally {
  ...
  micBtn.addEventListener("click", () => {
    if (window.listeningPracticeActive && window.listeningTargetSentence) {
      ...
      const targetLanguage =
        window.listeningTargetLanguage ||
      ...
      recognition.onresult = async function (event) {
        ...
        await evaluateListeningAttempt(
          attempt,
          window.listeningTargetSentence,
          targetLanguage,
        );
      };
      ...
      recognition.onerror = function (e) {
        ...
        addMessage(
          "⚠️ Could not recognize speech. Please try again.",
          "system-error",
        );
      };
...
if (listenBtn) {
  ...
  console.warn(
    "⚠️ Listening practice button not found in DOM. Make sure there's an element with id='listen-practice-btn'",
  );
}
