...
Automatically generate a vocabulary deck from your conversation history. Each card is scheduled using the **SM-2 algorithm** (the same spaced-repetition method behind Anki). Reviews are graded Again / Hard / Good / Easy, and the next-review date grows or shrinks based on how well you knew the card. A "Review Due Cards" view surfaces exactly what's due right now, with a live badge showing the due count.
...
| **Testing** | Jest |
| **CI/CD** | GitHub Actions (matrix testing, schema validation, dependency audit) |
...
│       ├── flashcards.js     # Flashcard interface + due-cards review mode
│       ├── spacedRepetition.js # SM-2 spaced repetition scheduling algorithm
